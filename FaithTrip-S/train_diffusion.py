...
from utils import load_flickr_data, split_df_leave_one_out
...
def main(opt):
    ...
    if opt.dataset_type == 'Flickr':
        ...
        print(f'total_mean_distance: {mean_dis}, total_distortion_distance: {distortion_dis}')

        if opt.wandb:
            wandb.finish()

        with open(
                f'./results/FaithTrip-S/{opt.dataset}/{opt.step}-{opt.max_noise}-{opt.lr}-{opt.batch_size}-{opt.d_model}-{opt.num_encoder_layers}.txt',
                'a') as file:
            file.write(f'===========================' + '\n')
            file.write(f'seed:{opt.seed}'+ '\n')
            file.write(f'learning rate: {opt.lr}'+ '\n')
            file.write(f'model dimension: {opt.d_model}'+ '\n')
            file.write('\n')
            ...
            file.write(f'distortion: {distortion_dis}'+ '\n')
...
if __name__ == '__main__':
    ...
    # setting
    parser.add_argument('--seed', type=int, default=2024, help='manual seed')
    ...
    # train:
    parser.add_argument('--step', type=int, default=32, help='diffusion_step')  # 32
    parser.add_argument('--max_noise', type=float, default=0.02, help='beta_t')  # 0.02
