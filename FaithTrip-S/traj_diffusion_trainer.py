...
class TrajDiffusionTrainer:
    ...
    def train(self):
        ...
        # calculate results
        best_index = count_best_index(epoch_f1_score_list, epoch_pairs_f1_list, epoch_distance_list)
        # pick the max f1 in each trajectory
        return (epoch_start_list[best_index], epoch_end_list[best_index], epoch_f1_score_list[best_index],
                epoch_pairs_f1_list[best_index], epoch_distance_list[best_index],
                epoch_distortion_distance_list[best_index])
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            for masked_padded_venue_ids, masked_padded_hour_ids, padded_venue_ids, padded_hour_ids, _, _ in \
                ...
                print("recommend:", predicted_ids[0], "target:", padded_venue_ids[0])
                # calculate the trajectory
                # with open("glas.txt", "a") as file:
                #     file.write(f"====================================== + \n")
                #     file.write(f"recommend: {predicted_ids[0]}, target: {padded_venue_ids[0]}")
