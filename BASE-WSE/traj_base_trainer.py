...
class TrajBaseTrainer:
    def __init__(self,
        ...
        # configuration
        # Ignore padding index during loss calculation
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for masked_padded_venue_ids, masked_padded_hour_ids, padded_venue_ids, _, _ in \
                ...
                # add the start and end weight
                ce_weights = np.full_like(masked_padded_venue_ids, 1)
                ce_weights[masked_padded_venue_ids != 0] = self.weight_num  # [b,l]
                ce_weights = torch.tensor(ce_weights)

                loss = torch.mean(self.criterion(venue_output.view(-1, self.venue_vocab_size),
                                                 padded_venue_ids.flatten()) * (ce_weights.flatten()))
            ...
            if self.data_type == 'Flickr':
                ...
                print(eval_loss)
        # calculate results
        f1_max_index = count_best_index(epoch_alt_f1_score_list, epoch_alt_pairs_f1_list, epoch_distance_list)
        f1_max_total_index = count_best_index(epoch_total_f1_score_list, epoch_total_pairs_f1_list, epoch_distance_list)
