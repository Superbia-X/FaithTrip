...
"""
This model (BASE) is a naive transformer-like model for trip planning, multiple steps
...
def main(opt):
    ...
    if opt.dataset_type == 'Flickr':
        ...
        with open(
                f'./results/BASE/{opt.dataset}/{opt.lr}-{opt.batch_size}-{opt.d_model}-{opt.num_encoder_layers}-{opt.num_epochs}.txt',
                'a') as file:
            file.write("===" * 18 + '\n')
            file.write(f'encoder-layer: {opt.num_encoder_layers}' + '\n')
